  ...
  const response=await fetch("https://api.openai.com/v1/responses",{
   ...
   body:JSON.stringify({
    ...
    input:[{role:"user",content:[
     {type:"input_text",text:"To są kolejne strony faktury. Każda strona zdjęciowa może być pokazana jako dwie następujące po sobie części: górna i dolna, które lekko na siebie zachodzą. Zachowaj kolejność stron i nie duplikuj wierszy widocznych w zakładzie. Odczytaj tabelę WIERSZ PO WIERSZU ze wszystkich części i stron. Kolumna Lp. jest jedynym wyznacznikiem nowej pozycji: utwórz dokładnie jeden rekord dla każdego wydrukowanego numeru Lp. Tekst bez własnego numeru Lp. jest kontynuacją nazwy wcześniejszej pozycji — połącz go z nią. Dla każdego Lp. patrz poziomo i odczytaj z tego samego wiersza pełną Nazwę towaru lub usługi, Ilość i Jedn.m. KOLUMNA ILOŚĆ WYMAGA PODWÓJNEJ KONTROLI: najpierw odczytaj cyfrę bezpośrednio z komórki Ilość, a potem niezależnie sprawdź ją przy użyciu kolumn Cena jedn. brutto i Wartość brutto (Ilość × cena jednostkowa = wartość brutto). Przykład: cena 25,00 i wartość 100,00 oznaczają ilość 4; cena 25,00 i wartość 75,00 oznaczają ilość 3. W razie różnicy wybierz ilość potwierdzoną rachunkiem. Uważaj szczególnie na pomylenie cyfr 3 i 4 oraz 1 i 2. Nie zakładaj, że ilości są jednakowe; sprawdź każdą pozycję osobno. Nie przesuwaj danych między wierszami. Nie zwracaj PKWiU, cen, wartości, netto, VAT ani brutto — służą wyłącznie do kontroli ilości. Jeśli ilości nie da się potwierdzić ani odczytać, pozostaw pusty tekst zamiast zgadywać. Jednostki zapisuj dokładnie, np. szt., kpl., mb. Na końcu wykonaj drugi przebieg wyłącznie po kolumnach Lp., Ilość, Cena jedn. brutto i Wartość brutto, popraw ilości sprzeczne z rachunkiem, a następnie sprawdź ciągłość Lp. oraz zgodność każdego wiersza."},...documentParts
    ]}],
